...
import com.example.consumeapi.ui.kontak.viewmodel.DetailKontakUiState
...
object DetailDestination : DestinasiNavigasi {
    override val route =  "item_details" //memberi nama setiap halaman
    override val titleRes = "Detail Kontak" //memberi caption topbar
...
@Composable
fun DetailStatus(
    kontakUIState: DetailKontakUiState,
    retryAction: () -> Unit,
    modifier: Modifier = Modifier,
    onEditClick: (Int) -> Unit
) {
    ...
    when(kontakUIState) {
        is DetailKontakUiState.Success -> {
        ...
        is DetailKontakUiState.Loading -> {
            OnLoading(modifier = modifier)
        }

        is DetailKontakUiState.Error -> {
